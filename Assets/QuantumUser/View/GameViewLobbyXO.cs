namespace Quantum
{
    ...
    public unsafe class GameViewLobbyXO : QuantumEntityViewComponent
    {
        ...
        private void OnPlayerListChanged(QList<EntityRef> players)
        {
            ...
            var playerArray = VerifiedFrame.Global->playerArray;
            SetActiveIconXO(false, EntityRef.None, iconX, iconO);
            ...
            if (QuantumRunner.Default.Game.Session.IsLocalPlayer(playerInfo.PlayerRef))
            {
                ...
                SetActiveIconXO(true, players[0], iconX, iconO);
            }
            else
            {
        ...
        private void SetActiveIconXO(bool isActive, EntityRef playerEntityRef, Sprite spriteX, Sprite spriteO)
        {
            if (!isActive)
            {
            ...
            else
            {
                ...
                for (int i = 0; i < lobbyUI.childCount; i++)
                {
                    ...
                    {
                        lobbyUI.GetChild(i).Find("ImageIcon").gameObject.SetActive(isActive);
                        lobbyUI.GetChild(i).Find("ImageIcon").GetComponent<Image>().sprite = spriteX;
                    }
                    if (lobbyUI.GetChild(i).Find("ImageAvt").gameObject.activeSelf && i != indexSlot)
                    {
                        lobbyUI.GetChild(i).Find("ImageIcon").gameObject.SetActive(isActive);
                        lobbyUI.GetChild(i).Find("ImageIcon").GetComponent<Image>().sprite = spriteO;
                    }
                }
            }
        }
